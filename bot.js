...
const commands = [
    ...
    new SlashCommandBuilder().setName("gamble").setDescription("Gamble your coins")
...
client.on("messageCreate", async message => {
    ...
    if (args.startsWith("gamble ")) {
        ...
        const finalResult = [BOT_EMOJIS.spin, BOT_EMOJIS.spin, BOT_EMOJIS.spin];

        // Animate slots individually
        for (let i = 0; i < 3; i++) {
            for (let j = 0; j < 6; j++) {
                if (j === 5) finalResult[i] = SLOT_SYMBOLS[Math.floor(Math.random() * SLOT_SYMBOLS.length)];
                else finalResult[i] = BOT_EMOJIS.spin;
                embed.setDescription(finalResult.join(" "));
        ...
        embed.setColor(win ? "Green" : "Red")
             .setFooter({ text: win ? `🎉 YOU WON ${amount * 2} coins!` : `💀 You lost ${amount} coins!` });
        await spinMessage.edit({ embeds: [embed] });
    }
});
        ...
        case "gamble":
            ...
            for (let i = 0; i < 3; i++) {
                for (let j = 0; j < 6; j++) {
                    ...
                    gambleEmbed.setDescription(finalResult.join(" "));
                    await msg.edit({ embeds: [gambleEmbed] });
                    await new Promise(r => setTimeout(r, 300));
                }
            }
            ...
            gambleEmbed.setColor(win ? "Green" : "Red")
                       .setFooter({ text: win ? `🎉 YOU WON ${amount * 2} coins!` : `💀 You lost ${amount} coins!` });
            await msg.edit({ embeds: [gambleEmbed] });
            break;
    }
